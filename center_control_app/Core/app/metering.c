...
//****************  映射 参数
uint16_t* p_Connection_Mode=NULL;
uint16_t* p_mb_metering_Address=NULL;			//	模块地址

//**************** 快速查询表
// 0: Modbus   1: Metering
uint16_t HoldingAddrTable[3][5][2] = {{{0x20,0xFFFF},{0x21,0x0002},{0x22,0x0006},{0x23,0x000C},{0x24,0xFFFF},},
{{0x20,0x0003},{0x21,0x0002},{0x22,0x002E},{0x23,0x002D},{0x24,0xFFFF},},
{{0x20,0x0003},{0x21,0x0002},{0x22,0x002E},{0x23,0x002D},{0x24,0xFFFF},}};
...
{
{0x10,0x2044,1},		{0x12,0xFFFF,1},		{0x14,0xFFFF,1000},	{0x16,0x101E,1000},
{0x18,0x1028,1000},	{0x1A,0x2012,10},		{0x1C,0x201A,10},		{0x1E,0xFFFF,10},
{0x20,0x202A,1000},	{0x22,0x2000,10},		{0x24,0x200C,1000},	{0x26,0x2002,10},
{0x28,0x200E,1000},	{0x2A,0x2004,10},		{0x2C,0x2010,1000},	{0x2E,0xFFFF,1},
},
{
{0x10,0x2044,1},		{0x12,0xFFFF,1},		{0x14,0xFFFF,1000},	{0x16,0x101E,1000},
{0x18,0x1028,1000},	{0x1A,0x2012,10},		{0x1C,0x201A,10},		{0x1E,0xFFFF,10},
{0x20,0x202A,1000},	{0x22,0x2006,10},		{0x24,0x200C,1000},	{0x26,0x2008,10},
{0x28,0x200E,1000},	{0x2A,0x200A,10},		{0x2C,0x2010,1000},	{0x2E,0xFFFF,1},
},
};
...
uint16_t DataAddr_Metering_to_Modbus(uint16_t addr)
{
	...
	if( (*p_Connection_Mode >= SINGLE_PHASE) && ( *p_Connection_Mode <= THREE_PHASE_FOUR_WIRE) )
		table_mode = (*p_Connection_Mode) - 1;
	else
		return 0xFFFF;
	...
	for(i=0; i<sum; i++)
	{
		if(HoldingAddrTable[table_mode][i][1] == addr)
			return HoldingAddrTable[table_mode][i][0];
	}
	...
	for(i=0; i<sum; i++)
	{
		if(InputAddrTable[table_mode][i][1] == addr)
			return InputAddrTable[table_mode][i][0];
	}
...
uint16_t DataAddr_Modbus_to_Metering(uint8_t fun_code, uint16_t addr)
{
	...
	if( fun_code == MB_FUNC_READ_HOLDING_REGISTER )
	{
		...
		for(i=0; i<sum; i++)
		{
			if(HoldingAddrTable[table_mode][i][0] == addr)
				return HoldingAddrTable[table_mode][i][1];
		}
	}
	else if( fun_code == MB_FUNC_READ_INPUT_REGISTER )
	{
		...
		for(i=0; i<sum; i++)
		{
			if(InputAddrTable[table_mode][i][0] == addr)
				return InputAddrTable[table_mode][i][1];
		}
	}
...
void Set_PowerSum(void)
{
	...
	high = Get_DataAddr_Value( MB_FUNC_READ_INPUT_REGISTER, 0x16);
	low = Get_DataAddr_Value( MB_FUNC_READ_INPUT_REGISTER, 0x17);
	positive = (high)<<16 | (low);
	
	high = Get_DataAddr_Value( MB_FUNC_READ_INPUT_REGISTER, 0x18);
	low = Get_DataAddr_Value( MB_FUNC_READ_INPUT_REGISTER, 0x19);
...
void Metering_RxData(uint8_t len)
{
	...
	{
		metering_addr = InputAddrTable[(*p_Connection_Mode)-1][Metering_PERIODIC_CNT][1];
...
uint16_t Metering_Get_polling_cnt(uint16_t cnt)
{
	...
	{
		if(InputAddrTable[table_mode][i][1] != 0xFFFF)
		{
			result = i;
			return result;
		}
...
void Metering_Receive_Init(void)
{
  ...
  HAL_UART_Receive_DMA(p_Metering_Module_Huart,Metering_DMABuff,METERING_RS485_RX_BUFF_SIZE);//打开串口DMA接收
	...
	if(*p_Connection_Mode > THREE_PHASE_FOUR_WIRE)
	{
		Set_DataAddr_Value( MB_FUNC_READ_HOLDING_REGISTER,  MB_DATA_ADDR_CONNECTION_MODE,  NO_MODE);
	}
	// 模块地址
	p_mb_metering_Address = Get_DataAddr_Pointer(MB_FUNC_READ_HOLDING_REGISTER,MB_DATA_ADDR_METERING_MODULE_ADDR);
	Check_Metering_Address(p_mb_metering_Address);
...
void Metering_Protocol_Analysis(void)
{
  ...
  QueueEvent = osMessageGet (MeteringQueueHandle, 0);
	if(QueueEvent.status == osEventMessage)
	{
		...
		{
			if(meter_addr == MB_DATA_ADDR_CONNECTION_MODE)//接线方式 内容不一样
			{
				if( value == 2)value = 1;
	...
	// 定期轮询
	if(Check_Connection_Mode(p_Connection_Mode))
	{
		table_mode = (*p_Connection_Mode) - 1;
		...
		if(Metering_PERIODIC_CNT != 0xFFFF)
		{
			...
			{
				Metering_Periodic_Polling(Metering_TxBuff, InputAddrTable[table_mode][Metering_PERIODIC_CNT][1]);
				Metering_UART_Send(Metering_TxBuff, METERING_RS485_TX_BUFF_SIZE);
...
uint8_t Check_Connection_Mode(uint16_t* p_mode)
{
	if( (*p_mode < SINGLE_PHASE) || ( *p_mode > THREE_PHASE_FOUR_WIRE) )
		*p_mode = THREE_PHASE_FOUR_WIRE;//默认 三相四线
